...
export function ClaimsListPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => claimsApi.list({ page, limit: 20, status, type }),
  });
  const rows = useMemo(
    () =>
      (query.data?.data ?? []).filter((claim) =>
        [claim._id, claim.claimNumber, claim.patientId, claim.hospitalId]
          .join(" ")
          .toLowerCase()
  ...
  const columns: Column<Claim>[] = [
    {
      key: "id",
      header: "Claim",
      cell: (c) => <strong>{c.claimNumber ?? c._id.slice(-8)}</strong>,
      sortValue: (c) => c.claimNumber ?? c._id,
    },
    {
      key: "patient",
  ...
  return (
    <div className="page-stack">
      ...
      <DataTable
        ...
        onRowClick={(row) => navigate(`/claims/${row._id}`)}
      />
    </div>
  );
}
