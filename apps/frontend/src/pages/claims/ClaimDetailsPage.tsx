...
export function ClaimDetailsPage() {
  ...
  return (
    <div className="claim-cockpit">
      <div className="cockpit-main">
        <div className="page-title">
          <p className="eyebrow">Claim command cockpit</p>
          <h1>{data.claimNumber ?? data._id}</h1>
          <span>
            {data.type} · {ageInDays(data.createdAt)} days ageing · Updated{" "}
        ...
        <section className="cockpit-strip">
          ...
          <div>
            <span>Hospital</span>
            <strong>{data.hospitalId}</strong>
          </div>
        </section>
        {locked && (
          <div className="audit-warning">
            Closed claim is read-only. Reopening requires SUPER_ADMIN
        ...
        <div className="two-column">
          ...
          <Card>
            <CardHeader
              ...
              eyebrow="Read-before-action"
            />
            <dl className="detail-list">
              <dt>Total claim</dt>
              <dd>{formatCurrency(data.totalClaimAmount)}</dd>
              <dt>TDS</dt>
              <dd>{formatCurrency(data.tdsAmount)}</dd>
              <dt>Deductions</dt>
              <dd>{formatCurrency(data.deductions)}</dd>
              <dt>Hospital discount</dt>
              <dd>{formatCurrency(data.hospitalDiscount)}</dd>
              <dt>Deposit</dt>
              <dd>{formatCurrency(data.depositAmount)}</dd>
              <dt>Refund</dt>
        ...
        <Card>
          <CardHeader
            ...
            eyebrow="Status · communications · documents · settlement · alerts · audit"
          />
          {timeline.isError ? (
        ...
        <Card>
          <CardHeader title="Documents" eyebrow="Secure claim file" />
          <DocumentManager claimId={data._id} />
        </Card>
        <Card>
          <CardHeader
            title="Settlement, allocations, deposits and refunds"
        ...
        <div className="two-column">
          <Card>
            ...
            >
              <select
                ...
                value={medium}
                onChange={(e) =>
                  setMedium(e.target.value as CommunicationMedium)
                }
              >
                <option>EMAIL</option>
            ...
            {communications.data?.data.map((c) => (
              <div className="alert-row" key={c._id}>
                <StatusBadge value={c.medium} compact />
                <span>
                  {c.remarks} · {formatDateTime(c.createdAt)}
                </span>
              </div>
            ))}
          </Card>
          <Card>
            <CardHeader
              title="Alerts and audit history"
              eyebrow="Risk visibility"
            />
            {alerts.data?.map((a) => (
              <div className="alert-row" key={a._id}>
                <StatusBadge value={a.severity} compact />
                <span>
                  {labelize(a.type)} · {a.message}
                </span>
              </div>
            ))}
            <hr />
