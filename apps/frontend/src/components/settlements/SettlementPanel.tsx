...
export function SettlementPanel({ claim }: { claim: Claim }) {
  ...
  const settlement = useQuery({
    queryKey: ["settlement", claim._id],
    queryFn: () => settlementApi.getByClaim(claim._id),
    retry: false,
  });
  ...
  const create = useMutation({
    ...
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["settlement", claim._id] }),
  });
  ...
  const deposit = useQuery({
    queryKey: ["deposit", claim._id],
    queryFn: () => depositApi.getByClaim(claim._id),
    retry: false,
  });
  if (data)
    return (
