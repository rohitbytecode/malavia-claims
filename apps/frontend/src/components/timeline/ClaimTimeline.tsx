...
import { formatDateTime, labelize, nameOf } from "../../utils/format";
    ...
    <div className="timeline">
      {sorted.map((event, index) => (
        <article
          ...
          key={event.id ?? event._id ?? `${event.type}-${index}`}
        >
          <div className="timeline-icon">{event.type[0]}</div>
          <div className="timeline-content">
            ...
            {event.fromStatus && event.toStatus && (
              <p>
                {labelize(event.fromStatus)} → {labelize(event.toStatus)}
              </p>
            )}
            <p>{event.remarks ?? event.message ?? "No remarks recorded"}</p>
            {event.attachmentName && (
              <span className="secure-file">
                Secure attachment: {event.attachmentName}
              </span>
            )}
          </div>
        </article>
