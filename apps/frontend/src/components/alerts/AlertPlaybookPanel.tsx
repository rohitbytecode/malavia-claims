...
export function AlertPlaybookPanel({ alerts, role }: { alerts: Alert[]; role?: Role }) {
  ...
  return (
    <section className="playbook-panel premium-panel">
      <div className="panel-title-row">
        ...
        <strong>{sorted.filter((alert) => !alert.resolved).length} active</strong>
      </div>
      <div className="playbook-list">
        {sorted.slice(0, 6).map((alert) => {
          ...
          return (
            <article className={`playbook-item ${owner ? "owned" : ""}`} key={alert._id}>
