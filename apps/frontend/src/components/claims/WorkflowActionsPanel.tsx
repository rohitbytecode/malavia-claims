...
export function WorkflowActionsPanel({ claim }: { claim: Claim }) {
  ...
  const transitions = allowedTransitions(claim.type, claim.status).filter(
    (status) => (user ? canRoleTransition(user.role, status) : false)
  );
  ...
  return (
    <aside className="action-panel">
      ...
      {closed && !canReopen(user?.role) && (
        <div className="restriction">
          Closed claims are locked. Only SUPER_ADMIN may reopen with audit
          reason.
        </div>
      )}
      {closed && canReopen(user?.role) && (
        <Button variant="danger" onClick={() => setTarget("DRAFT")}>
      ...
      {transitions.length === 0 && !closed && (
        <div className="restriction">
          No permitted frontend workflow actions for your role at this status.
        </div>
      )}
      <Modal
        open={Boolean(target)}
      ...
      >
        <div className="modal-body">
          <p>
            Confirm transition to <strong>{labelize(target)}</strong>. The
            backend will enforce workflow validity and this reason supports
            audit review.
          </p>
          <textarea
            ...
            placeholder="Audit reason / operational remarks"
          />
          {remarks.length > 0 && remarks.length < 5 && (
            <small className="field-error">Enter at least 5 characters.</small>
          )}
          ...
          <div className="modal-actions">
            ...
            <Button
              disabled={!target || remarks.length < 5 || mutation.isPending}
              onClick={() =>
                target && mutation.mutate({ status: target, reason: remarks })
              }
            >
